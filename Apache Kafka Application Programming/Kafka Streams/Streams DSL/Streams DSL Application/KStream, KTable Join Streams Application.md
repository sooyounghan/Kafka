...
1. KTable과 KStream은 메세지 키를 기준으로 조인할 수 있음
   - 대부분 데이터베이스는 정적으로 저장된 데이터를 조인하여 사용햇지만, 카프카에서는 실시간으로 들어오는 데이터들을 조인 가능
   - 사용자의 이벤트 데이터를 데이터베이스에 저장하지 않고도 조인하여 스트리밍 처리 가능하다는 장점 존재
   - 이를 통해 이벤트 기반 스트리밍 데이터 파이프라인을 구성할 수 있음

2. 이름을 메세지 키, 주소를 메세지 값으로 가지고 있는 KTable이 있고, 주무할 물품을 메세지 값으로 가지고 있는 KStream이 존재한다고 가정
...
3. 만약, 사용자의 주소가 변경되는 경우에는 KTable은 동일한 메세지 키가 들어올 경우 가장 마지막 레코드를 유효한 레코드로 보므로 가장 최근에 바뀐 주소로 조인 수행
<div align="center">
<img src="https://github.com/user-attachments/assets/bdd2ef0a-a422-4f55-b3dd-54e80a7a750c" />
</div>

4. 스트림 데이터 join을 위한 토픽 생성
```
./bin/windows/kakfa-topics.bat --create --booystrap-server my-kafka:9092 --partition 3 --topic address
./bin/windows/kakfa-topics.bat --create --booystrap-server my-kafka:9092 --partition 3 --topic order
./bin/windows/kakfa-topics.bat --create --booystrap-server my-kafka:9092 --partition 3 --topic order_join
```
<div align="center">
